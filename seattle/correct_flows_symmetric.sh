...
ovs-ofctl -O Openflow13 add-flow br-int "table=11, tcp,reg0=0x1,tp_dst=$block_port actions=move:NXM_NX_TUN_ID[0..31]->NXM_NX_NSH_C2[],push_nsh,load:0x1->NXM_NX_NSH_MDTYPE[],load:0x3->NXM_NX_NSH_NP[],load:$nsp_sym->NXM_NX_NSH_C3[],load:$ip_compute->NXM_NX_NSH_C1[],load:$nsp->NXM_NX_NSP[0..23],load:0xff->NXM_NX_NSI[],load:0xc0a8002b->NXM_NX_TUN_IPV4_DST[],load:$nsp->NXM_NX_TUN_ID[0..31],resubmit($vxlan_gpe,0)"

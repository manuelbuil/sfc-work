...
do
neutron security-group-rule-create --direction egress --ethertype IPv4 --protocol udp --port-range-min 53 --port-range-max 53 $i
done
...
sshpass -p opnfv ssh $ssh_options root@$ip_client "sed -i 's/11.0.0.2/192.168.0.6/g' /etc/resolv.conf"
